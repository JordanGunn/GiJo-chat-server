...
#include "../include/tcp_init_client.h"
...
struct addrinfo * tcp_client_addr(const char * host, const char * port)
{
    ...
    if ( (result = getaddrinfo(host, port, &hints, &serv_info)) == -1 )
    {
...
void tcp_client_send(int sock_fd, uint8_t * data);
{
    ...
    bytes_sent = send(sock_fd, init_params, strlen(init_params), 0);
    if (bytes_sent < 0)
    {
